...

{
...

    {
        ...
        /// <summary>
        /// NEW METHOD: Counts clearance records matching a specific department and status value.
        /// Resolves the compilation error inside BaseOfficeForm.cs.
        /// </summary>
        public int GetRequestCountByStatus(string officeName, string status)
        {
            using (var db = dbManager.GetConnection())
            {
                // DEBUG: First let's see ALL distinct Department values in your table
                string debugSql = "SELECT DISTINCT Department FROM ClearanceRequests";
                var allDepts = db.Query<string>(debugSql);
                string deptList = string.Join(", ", allDepts);

                // Show what's in DB vs what we're searching for
                System.Windows.Forms.MessageBox.Show(
                    $"Searching for OfficeName: '{officeName}'\n" +
                    $"Status: '{status}'\n" +
                    $"Departments in DB: {deptList}",
                    "Debug");
